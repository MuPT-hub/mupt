...
def optional_in_place(
    ...
    mod_type = tuple(old_sig.parameters.values())[
        0
    ].annotation  # get annotation of the object being modified
    in_place_wrapper.__signature__ = new_sig.replace(
        return_annotation=Optional[mod_type]
    )  # replace signature with appropriate modifications
...
def flexible_listlike_input(
    ...
    def wrapper(
        *args,
    ) -> T:  # wrapper which accepts an arbitrary number of non-keyword argument
