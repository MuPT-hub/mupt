...
Decorator: TypeAlias = Callable[
    [Callable[Params, ReturnType]], Callable[Params, ReturnType]
]
...
def extend_to_methods(dec: Decorator) -> Decorator:
    ...
    # transfer decorator signature to decorator adapter class
    # BUT without updating the __dict__ field
    @wraps(dec, updated=())
    class AdaptedDecorator:
        ...
        ) -> Callable[[Concatenate[object, Params]], ReturnType]:
            """
            Generate partial application with calling instance
            as first argument (fills in for 'self')
            """
            method = self.funct.__get__(
                instance, owner
            )  # look up method belonging to owner class
