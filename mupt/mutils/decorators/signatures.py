...
KEYWORD_PARAMETER_TYPES = (Parameter.KEYWORD_ONLY, Parameter.VAR_KEYWORD)
...
def insert_parameter_at_index(
    sig: Signature, new_param: Parameter, index: int
) -> Signature:
...
def modify_param_annotation_by_name(
    ...
    return modify_param_annotation_by_index(
        ...
        index=list(sig.parameters.keys()).index(
            param_name
        ),  # look up location of target parameter
        new_type=new_type,
    )
