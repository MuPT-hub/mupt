...
def is_empty_file(filepath: Path) -> bool:
    ...
    if filepath.is_dir():
        # NOTE: not checking file existence here, as calling stat()
        # will already do this check (and raise appropriate error)
        raise IsADirectoryError(
            f"filepath must point to file, not to directory '{filepath}'"
        )
...
def assemble_path(
    ...
    and filetype info to generate a complete Path
    """
    if extension[0] == ".":
...
def default_suffix(path: Path, suffix: str) -> Path:
    """
    Asserts that a path has a suffix, appending
    a specified default suffix if none exists
    """
    if not path.suffix:
        path = path.with_name(
            f"{path.stem}.{suffix}"
        )  # ensure charge params path has correct extension
