...
def resolve_module_name(module: Optional[ModuleType | str]) -> str:
    """
    Given a ModuleType, returns a str-type name for
    it based on its ModuleSpec (or lack thereof)
    """
...
def module_parts(module: ModuleType | str) -> tuple[Optional[str], str]:
    """
    Takes a module (as its name or as ModuleType) and
    returns its parent package name and relative module name
    """
    module_name: str = resolve_module_name(module)
    parent_package_name, _, module_stem = module_name.rpartition(
        "."
    )  # split on rightmost dot separator
...
def module_stem(module: ModuleType | str) -> str:
    """
    Takes a module (as its name or as a ModuleType)
    and returns its relative module name
    """
...
def relative_module_name(
    ...
    If the given module is not in the path of the toplevel
    module, will simply return as module.__name__
    """
