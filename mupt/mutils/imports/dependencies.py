...
class MissingPrerequisitePackage(Exception):
    ...
    def __init__(
        ...
        dependency_name_formal : Optional[str], default None
            An optional stylized version of the module
            name, if different to how it appears in code
        ...
        use_case : str, default 'import'
            An optional specific reason the caller
            might want to import the dependent code
        ...
        importing_package_name : Optional[str], default None
            An optional means of configuring which importing
            module is displayed in the error message

            Defaults to module in which the caller is located, determined automatically
        """
        ...
        if not importing_package_name:
            ...
            importing_package_name = resolve_module_name(
                get_calling_module(levels_above=1)
            )

        install_desc: str = (
            f" by following the installation instructions at {install_link}\n"
            if install_link
            else ""
        )

        message = (
            f"{use_case.capitalize()} require(s) {dependency_name_formal}, "
            "which was not found in the current context.\n"
            f"Please install `{dependency_name}`{install_desc}, then "
            f"try importing from '{importing_package_name}' again"
        )
...
def module_installed(module_name: str) -> bool:
    ...
    module_name : str
        ...
        Does not support direct passing of module objects to avoid circularity;
        No reason to check if a module is present if its already imported elsewhere

    Returns
    -------
    module_found : bool
        Whether the module was found to be installed in the current working environment
    """
    # try:
    ...
    try:
        # NOTE: opted for this implementation, as it never actually
        # imports the package in question (faster and fewer side-effects)
...
def modules_installed(*module_names: str) -> bool:
    ...
    all_modules_found : bool
        Whether all modules were found installed in the current working environment
    """
...
def requires_modules(
    *required_module_names: str,
    ...
    if not isinstance(missing_module_error, Exception):
        if not (
            isinstance(missing_module_error, type)
            and issubclass(missing_module_error, Exception)
        ):
            # DEV: this is potentially brittle, depending on
            # how the specific Exception subtype is implemented?
            raise TypeError(
                "Must pass either an Exception instance or "
                "an Exception subtype to 'missing_module_error'"
            )
    ...
    def tailored_exception(module_name: str) -> Exception:
        ...
        if isinstance(missing_module_error, type):
            return missing_module_error(
                f'No installation found for module "{module_name}"'
            )
