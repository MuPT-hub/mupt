...
def generate_amino_acid_substructures() -> set[AminoAcidSubstructure]:
    ...
    for letter, ptabmol in AMINO_ACID_CODES.items():
        ...
        if (tripeptide is None) or (tripeptide.GetNumAtoms() == 0):
            LOGGER.debug(
                f'RDKit has no amino acid registered to FASTA code "{letter}"; skipping'
            )
            continue
        pdb_3_letter_code: str = (
        ...
        for match in tripeptide.GetSubstructMatches(PEPTIDE_BOND_QUERY):
            ...
            )  # implicitly enforces that we expect match to occur on pairs of atoms
            for map_num, atom_idx in zip((1, 2), match):
                tripeptide.GetAtomWithIdx(atom_idx).SetAtomMapNum(
                    map_num
                )  # label edge atoms with appropriate traversal direction map numbers
        ...
        cleaved_tripeptide = FragmentOnBonds(
            ...
            dummyLabels=[
                (1, 2),
                (1, 2),
            ],  # consistent with RETRO-ANTERO labels from TraversalDirection
        )
