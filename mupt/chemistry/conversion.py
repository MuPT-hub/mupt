...
def element_to_rdkit_atom(element: ElementLike) -> Atom:
    ...
    if not isatom(element):
        raise ValueError(
            "Expected an ElementLike instance, got"
            f"object of type {type(element).__name__}"
        )
...
def rdkit_atom_to_element(atom: Atom) -> ElementLike:
    ...
    # fetch Ion instance - NOTE: order here is deliberate;
    # can't fetch Isotope of Ion, but CAN fetch Ion of Isotope
