...
def element_to_rdkit_atom(element: ElementLike) -> Atom:
    ...
    if not isatom(element):
        raise ValueError(
            f"Expected an ElementLike instance, got object of type {type(element).__name__}"
        )

    atom = Atom(element.number)
    atom.SetNoImplicit(
        True
    )  # prevent RDKit from adding on any Hs where we don't expect
    atom.SetFormalCharge(element.charge)
    if hasattr(
        element, "isotope"
    ):  # covers case for charged isotopes, with are Ion (not Isotope) instances
...
def rdkit_atom_to_element(atom: Atom) -> ElementLike:
    ...
    atom_is_linker: bool = (
        atom.GetAtomicNum() == 0
    )  # DEV: am well-aware is_linker(atom : Atom) exists in the RDKit interface ("I wrote the damn bill"), but that import here would be circular
...
def flexible_elementlike(elem: Union[int, str, Atom, ElementLike]) -> ElementLike:
    ...
    if isatom(elem):
        ...
        raise TypeError(
            f"Cannot interpret object of type {type(elem).__name__} as ElementLike"
        )
