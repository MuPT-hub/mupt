...
def _check_connectors_cover_topology(
    ...
    * The image of each node has no fewer Connectors than the node has neighbors
    ...
    for node in topology.nodes:
        if (num_connectors := len(mapped_connectors[node])) < (
...
def deduce_connections_from_topology(  # noqa: C901
    ...
    while (n_iter < n_iter_max) and unpaired_edges:
        ...
        for edge_labels in unpaired_edges:
            ...
            conn_partition_former: set[frozenset[Connector]] = conn_partitions[
                node_label_former
            ]
            conn_partition_latter: set[frozenset[Connector]] = conn_partitions[
                node_label_latter
            ]
    ...
    if any(unpaired_edges):
        ...
        LOGGER.info(
            "Linking protocol successful! "
            f"{num_total_edges - len(unpaired_edges)}/{num_total_edges} "
            "edges were assigned Connector pairings"
        )
...
def assign_connections_from_topology(
    ...
    of Connectors and assign neighborship based on it
    """
