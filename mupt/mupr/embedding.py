...
class ConnectorReference:
    ...
    def with_reassigned_primitive(
        self, new_primitive_handle: PrimitiveHandle
    ) -> "ConnectorReference":
...
def infer_connections_from_topology( # noqa: C901
    ...
    unpaired_edges: set[frozenset[PrimitiveHandle]] = set(
    ...
    for owner_handle, connector_map in mapped_connectors.items():
        ...
        connector_equiv_classes[owner_handle] = {
            i: set(equiv_class) for i, equiv_class in enumerate(equiv_class)
        }
    ...
    while (n_iter < n_iter_max) and unpaired_edges:
        ...
        for edge_labels in unpaired_edges:
            ...
            for class_label, owner_label in zip(compatible_class_labels, edge_labels):
                ...
                if (
                    len(equiv_class) == 0
                ):  # remove bin from equivalence classes if empty after drawing
                    _ = connector_equiv_classes[owner_label].pop(class_label)
            paired_connectors[frozenset(edge_labels)] = frozenset(
