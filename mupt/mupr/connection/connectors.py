...
class Connector(
    ...
    def __init__(
        ...
        # DEV: no call to setter; must be assigned via protected tangent_vector property
    ...
    # DEV: necessary for bw compat w/ attr refs, though could be deprecated eventually
    ## Bond vector
    @property
    ...
    def coincides_with(self, other: "Connector") -> bool:
        ...
        return (  # TODO: set atol/rtol for float vector comparison
            ...
            and compare_optional_positions(
                self._tangent_position, other._tangent_position
            )
        )
    ...
    def bondable_connector_pairs(
        ...
        # TB: is order-sensitivity still necessary, now that Connector
        # comes equipped with the "holder" reverse-lookup?
        # Only reason for having these fundamentally unordered pairs
        ...
        Pairs are sensitive to the order of connector collections, and the returns
        are equivariant in such, i.e. if bondable_connector_pairs(C0, C1) contains
        the pair (ca, cb), then bondable_connector_pairs(C1, C0) will contain the
        corresponding pair (cb, ca) instead
        """
        # Cut down full product space search by only considering representatives
        # of equivalence classes of Connectors which are geometrically and
        # selectivity-wise interchangeable
    ...
    def individualize(
        self,
    ) -> dict[tuple[AttachmentLabel, AttachmentLabel], "Connector"]:
        """
        ...
        """
        indiv_conn_map = dict()
        for anchor_label, linker_label in cartesian(
    ...
    def counterpart(self) -> "Connector":
        """
        Create a counterpart Connector which is identical to
        this Connector but has its linker and anchor sites swapped

        By construction, the counterpart will always be bondable with
        this Connector (and vice versa), assuming the attachables set of
        the anchor and linker point are both non-empty
        """
        counterpart = self.copy()
        counterpart.anchor, counterpart.linker = self.linker, self.anchor
        if self.has_tangent_position:
            # NOTE: since this vector is defined by difference to tangent point,
            # updated tangent point can be set directly from this difference,
            # since the anchor is updated about the origin
    ...
    def label(self, new_label: ConnectorLabel) -> None:
        ...
        if not isinstance(new_label, Hashable):
            raise TypeError(
                f"Connector label must be a Hashable type, not {type(new_label)}"
            )
    ...
    def __repr__(self) -> str:
        ...
        attr_str = ", ".join(
            f"{attr}={value!r}" for (attr, value) in repr_attr_strs.items()
        )
