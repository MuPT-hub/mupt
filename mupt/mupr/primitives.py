...
class Primitive(NodeMixin, RigidlyTransformable):
    ...
    def element(self, new_element: ElementLike) -> None:
        if self.children:
            raise AtomicityError(
                'Primitive with non-trivial internal structure cannot be made atomic (i.e. cannot have "element" assigned)'
            )
    ...
    def check_valence(
        self,
    ) -> None:  # DEV: deliberately put this here (i.e. not next to "valence" def) for eventual peelaway when splitting off AtomicPrimitive
        ...
        if not valence_allowed(self.element.number, self.element.charge, self.valence):
            raise ValueError(
                f"Atomic {self._repr_brief(include_functionality=True)} with total valence {self.valence} incompatible with assigned element {self.element!r}"
            )
    ...
    def fetch_connector(self, connector_handle: ConnectorHandle) -> Connector:
        ...
        try:
            ...
            raise MissingConnectorError(
                f'No Connector with handle "{connector_handle}" bound to {self._repr_brief()}'
            )
        ...
        * are bondable with each other
        ...
        if conn_ref1.primitive_handle == conn_ref2.primitive_handle:
            raise IncompatibleConnectorError(
                f'Child primitive "{conn_ref1.primitive_handle}" cannot be connected to itself'
            )
    ...
    def bind_external_connector(
        ...
        LOGGER.debug(
            f'Added Connector "{own_conn_handle}" as counterpart to Connector "{child_connector_handle}" on child Primitive tagged "{child_handle}"'
        )
    ...
    def unbind_external_connector(self, connector_handle: ConnectorHandle) -> Connector:
        ...
        if connector_handle not in self._external_connectors:
            raise UnboundConnectorError(
                f'Connector "{connector_handle}" bound to {self._repr_brief()} exists, but is not associated with the Connector of any child Primitive'
            )
    ...
    def connector_trace(
        self, connector_handle: ConnectorHandle
    ) -> list[
        Connector
    ]:  # DEV: eventually, make wrapping type set, once figured out how to hash Connectors losslessly
        """
    ...
    def check_external_connector_references_valid(self) -> None:
        ...
        if self.functionality != len(self.external_connectors):
            raise BijectionError(
                f"{self._repr_brief(include_functionality=True)} only has {len(self.external_connectors)} registered external Connectors"
            )
    ...
    @property
    def unique_child_labels(
        self,
    ) -> set[
        PrimitiveLabel
    ]:  # NOTE: this type annotation SHOULD be from PrimitiveLabel (NOT PrimitiveHandle!)
    ...
    def fetch_child(self, primitive_handle: PrimitiveHandle) -> "Primitive":
        ...
        try:
            ...
            raise MissingSubprimitiveError(
                f'No child Primitive with handle "{primitive_handle}" bound to {self._repr_brief()}'
            )
    ...
    def attach_child(
        ...
        LOGGER.info(
            f'Attached child Primitive "{subprim_handle}" to parent Primitive "{self._repr_brief()}"'
        )
    ...
    def _post_attach(self, parent: "Primitive") -> None:
        ...
        LOGGER.debug(
            f"Primitive {parent._repr_brief()} assigned as parent of Primitive {self._repr_brief()}"
        )
    ...
    def detach_child(
        ...
        assert (
            ...
            f'Failed to track all external connections to child Primitive "{target_handle}"'
        )
        ...
        LOGGER.info(
            f'Detached child Primitive "{target_handle}" from parent Primitive "{self._repr_brief()}"'
        )
    ...
    def _post_detach(self, parent: "Primitive") -> None:
        """Post-actions to take once attachment is verified and parent is bound"""
        LOGGER.debug(
            f"Primitive {parent._repr_brief()} disowned former child Primitive {self._repr_brief()}"
        )
    ...
    def disconnect_children(
        ...
        if internal_conn_ref is None:
            LOGGER.warning(
                "No internal connections exist between the given pair of children; nothing to disconnect"
            )
            return
    ...
    def check_children_bijective_to_topology_nodes(
        ...
        if topology.number_of_nodes() != self.num_children:
            raise BijectionError(
                f"Cannot bijectively map {self.num_children} child Primitives onto {topology.number_of_nodes()}-element topology"
            )
    ...
    def check_internal_connections_bijective_to_topology_edges(
        ...
        if topology.number_of_edges() != self.num_internal_connections:
            raise BijectionError(
                f"Cannot bijectively map {self.num_internal_connections} internal connections onto {topology.number_of_edges()}-edge topology"
            )
    ...
    def check_topology_compatible(
        self, topology: Optional[TopologicalStructure] = None
    ) -> None:
        """
    ...
    def adjoin_child_nodes(
        ...
        **edge_attrs,
        ...
        LOGGER.debug(
            f'Added edge between child Primitives "{child_1_handle}" and "{child_2_handle}" in topology of {self._repr_brief()}'
        )
    ...
    def expand(
        ...
        LOGGER.info(
            f'Expanding child Primitive "{target_handle}" of {self._repr_brief()}, replacing it with its {child_primitive.num_children} children'
        )
        ...
        for (
            ...
            handle_remap[old_handle] = self.attach_child(
                grandchild, label=old_label
            )  # attach and map handle - # DEV: worth explicitly detaching grandchildren from target?
    ...
    def shape(self, new_shape: BoundedTransformableShape) -> None:
        ...
        if not isinstance(new_shape, BoundedTransformableShape):
            raise TypeError(
                f"Primitive shape must be BoundedTransformableShape instance, not object of type {type(new_shape).__name__}"
            )
    ...
    def canonical_form_shape(
        self,
    ) -> str:  # DEVNOTE: for now, this doesn't need to be abstract (just use type of Shapefor all kinds of Primitive)
        """A canonical string representing this Primitive's shape"""
        return type(
            self.shape
        ).__name__  # TODO: move this into .shape - should be responsibility of individual Shape subclasses

    def canonical_form(
        self,
    ) -> str:  # NOTE: deliberately NOT a property to indicated computing this might be expensive
