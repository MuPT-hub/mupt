...
class Primitive(NodeMixin, RigidlyTransformable):
    ...
    shape : Optional[BoundedTransformableShape]
        A rigid shape which approximates and abstracts
        the behavior of the primitive in space
    element : Optional[Union[Element, Ion, Isotope]]
        The chemical element associated with this Primitive,
        IFF the Primitive represents an atom
    ...
    def num_atoms(self) -> int:
        """
        Number of atomic Primitives collectively present
        below this Primitive in the hierarchy
        """
    ...
    # DEV: deliberately put this here (i.e. not next to "valence" def)
    # for eventual peelaway when splitting off AtomicPrimitive
    def check_valence(self) -> None:
        ...
        if not valence_allowed(self.element.number, self.element.charge, self.valence):
            raise ValueError(
                f"Atomic {self._repr_brief(include_functionality=True)} "
                f"with total valence {self.valence} incompatible with "
                f"assigned element {self.element!r}"
            )
    ...
    def valence(self) -> int:
        """
        Electronic valence of the Primitive, i.e. the total
        bond order of all external-facing Connectors on this Primitive
        """
        total_bond_order: float = sum(
            BOND_ORDER.get(conn.bondtype, 0.0) for conn in self._connectors.values()
        )
    ...
    def register_connector(
        ...
        if not isinstance(new_connector, Connector):
            raise TypeError(
                f"Cannot interpret object of type {type(new_connector)} as Connector"
            )
        # TODO: bind new connectors externally to parent
    ...
    def fetch_connector(self, connector_handle: ConnectorHandle) -> Connector:
        ...
        try:
            ...
            raise MissingConnectorError(
                f"No Connector with handle '{connector_handle}' "
                f"bound to {self._repr_brief()}"
            )

    @overload
    def fetch_connector_on_child(
        self, primitive_handle: ConnectorReference
    ) -> Connector: ...

    @overload
    def fetch_connector_on_child(
        self, primitive_handle: PrimitiveHandle, connector_handle: ConnectorHandle
    ) -> Connector: ...
    ...
    def fetch_connector_on_child(
        ...
        return self.fetch_child(conn_ref.primitive_handle).fetch_connector(
    ...
    def all_connectors_on_children(self) -> frozenset[ConnectorReference]:
        """
        Unordered collection of all Connectors on children,
        associated as ConnectorReference instances
        """
        return frozenset({
    ...
    @property
    def internal_connections_by_pairs(
        self,
    ) -> dict[frozenset[PrimitiveHandle], frozenset[ConnectorReference]]:
        """
        Map from unordered pairs of child Primitive handles to unordered pairs
        of Connector references between that pair of children
        """
        # NOTE: prevents more than one pair between given children,
        # enforcing a "no-multigraph" attitude
        return {
            frozenset(
                conn_ref.primitive_handle for conn_ref in connected_pair
            ): connected_pair
    ...
    # DEV: this is potentially confusing/easily mixed up
    # w/ "num_internal_connections" - revisit naming
    @property
    ...
    def internal_connection_between(
        ...
        (from_child's connector handle, to_child's connector handle)
        """
        for from_child_conn_handle, conn_ref in self.internal_connections_on_child(
            from_child_handle
        ).items():
    ...
    def neighbor_handles(self, child_handle: PrimitiveHandle) -> set[PrimitiveHandle]:
        """
        Set of handles of all sibling child Primitives
        directly connected to the given child Primitive
        """
        return set(
    ...
    def check_internally_connectable(
        self,
        ...
        * are bondable with each other
        """
        if (prim_handle1 := conn_ref1.primitive_handle) == conn_ref2.primitive_handle:
            raise IncompatibleConnectorError(
                f'Child primitive "{prim_handle1}" cannot be connected to itself'
            )

        # performs necessary existence checks for
        # children and their Connectors while fetching
        ...
        if not Connector.bondable_with(conn1, conn2):
            raise IncompatibleConnectorError(
                f"Connector {conn_ref1.connector_handle} "
                f"on Primitive {conn_ref1.primitive_handle} "
                "is not bondable with "
                f"Connector {conn_ref2.connector_handle} "
                f"on Primitive {conn_ref2.primitive_handle}"
            )
    ...
    def pair_connectors_internally(
        ...
        """
        Associate a pair of Connectors between two adjacent
        children to the edge joining those children
        """
    ...
    def external_connectors(self) -> dict[ConnectorHandle, ConnectorReference]:
        """
        Mapping between the Connector handles found on
        self and their analogues on child Primitives
        """
        return self._external_connectors

    # TODO: find way to cache this (requires some guarantee of immutability of children)
    @property
    def external_connectors_by_children(
        self,
    ) -> dict[PrimitiveHandle, dict[ConnectorHandle, ConnectorHandle]]:
        """
        Mapping from child Primitive handles to (child Connector, own Connector)
        handle pairs defined by the external Connector map
        """
        ...
        for own_conn_handle, child_conn_ref in self.external_connectors.items():
            ext_conn_by_child[child_conn_ref.primitive_handle][
                child_conn_ref.connector_handle
            ] = own_conn_handle
    ...
    def num_external_connectors_on_child(self, child_handle: PrimitiveHandle) -> int:
        """
        Number of external connections a given child
        Primitive has mirrored by its parent (self)
        """
    ...
    def bind_external_connector(
        ...
        LOGGER.debug(
            f"Added Connector '{own_conn_handle}' as counterpart to "
            f"Connector '{child_connector_handle}' "
            f"on child Primitive tagged '{child_handle}'"
        )
    ...
    def unbind_external_connector(self, connector_handle: ConnectorHandle) -> Connector:
        ...
        if connector_handle not in self._external_connectors:
            raise UnboundConnectorError(
                f"Connector '{connector_handle}' bound to {self._repr_brief()} exists, "
                "but is not associated with the Connector of any child Primitive"
            )
        del self._external_connectors[connector_handle]

        return own_conn

    # DEV: eventually, make wrapping type set, once
    # figured out how to hash Connectors losslessly
    def connector_trace(self, connector_handle: ConnectorHandle) -> list[Connector]:
        ...
        if not self.is_leaf:
            # recursively trace downwards - this is the reason for not validating
            # the precondition recursively (duplicates effort done here)
            child_conn_ref: ConnectorReference = self.external_connectors[
                connector_handle
            ]
            child = self.fetch_child(child_conn_ref.primitive_handle)
            ext_conn_traces.extend(
                child.connector_trace(child_conn_ref.connector_handle)
            )
    ...
    def check_external_connector_references_valid(self) -> None:
        """
        Check that the mapped Connectors on self are each represented in
        the mapping to the associated external Connectors on children
        """
        if self.is_leaf:
        ...
        if self.functionality != len(self.external_connectors):
            raise BijectionError(
                f"{self._repr_brief(include_functionality=True)} only has "
                f"{len(self.external_connectors)} registered external Connectors"
            )
        ...
        if own_conn_handles != mapped_conn_handles:
            raise UnboundConnectorError(
                f"Connector mapping on {self._repr_brief()} is inconsistent; "
                f"{len(own_conn_handles - mapped_conn_handles)} Connector(s) have no "
                f"associated Connectors among children, and "
                f"{len(mapped_conn_handles - own_conn_handles)} mapped Connector(s) "
                "are not registered to the Primitive"
            )

    def check_internal_connection_references_valid(self) -> None:
        """
        Check that each pair of internal connections references
        a pair of Connectors which exist on their respective child Primitive
        """
    ...
    def check_connector_balance(self) -> None:
        ...
        if num_total_child_connectors != (
            self.num_internal_connectors + len(self.external_connectors)
        ):
            raise BijectionError(
                f"{self._repr_brief()} has Connectors unaccounted for; "
                f"{num_total_child_connectors} total vs {self.num_internal_connectors} "
                "internal + {len(self.external_connectors)} external Connectors"
            )
    ...
    def check_connectors(self) -> None:
        ...
        """
        # perform quick counting check to rule out necessarily-impossible cases
        self.check_connector_balance()
        self.check_external_connector_references_valid()
        self.check_internal_connection_references_valid()

    def check_child_referenced_faithfully(
        self, primitive_handle: PrimitiveHandle
    ) -> None:
        """
        ...
        """
        # DEV: looping over all children and calling this check is a less-efficient
        # way of checking self-consistency than the global check_connectors and
        # check_topology_consistent methods provided elsewhere
        ...
        if num_child_connectors != (num_external_on_child + num_internal_on_child):
            child_repr: str = child._repr_brief(
                include_functionality=True,
                label_to_use=primitive_handle,
            )
            raise BijectionError(
                f"Connectors on child {child_repr} not fully accounted for "
                f"(c.f. {num_child_connectors} total vs {num_internal_on_child} "
                f"internal + {num_external_on_child} external Connectors)"
            )

        # 2) check topology
        if (
            child_degree := self.topology.degree[primitive_handle]
        ) != num_internal_on_child:
            raise BijectionError(
                f"Primitive '{primitive_handle}' has {num_internal_on_child} "
                f"registered internal connections versus {child_degree} "
                "internal connections suggested by topology"
            )
    ...
    # NOTE: return type annotation SHOULD be from PrimitiveLabel (NOT PrimitiveHandle!)
    @property
    ...
    def child_exists(self, primitive_handle: PrimitiveHandle) -> bool:
        """Verify that referenced child Primitive is actually bound to this Primitive"""
    ...
    def fetch_child(self, primitive_handle: PrimitiveHandle) -> "Primitive":
        ...
        try:
            ...
            raise MissingSubprimitiveError(
                f"No child Primitive with handle '{primitive_handle}' "
                f"bound to {self._repr_brief()}"
            )
    ...
    def _pre_attach(self, parent: "Primitive") -> None:
        ...
        # DEV: insert any preconditions beyond checking parent is
        # self or one of self's children (already done by NodeMixin)
        ...
    ...
    def attach_child(
        ...
        neighbor_connections: Optional[
            dict[ConnectorHandle, tuple[PrimitiveHandle, ConnectorHandle]]
        ] = None,
    ) -> PrimitiveHandle:
        """
        ...
        # node addition is idempotent, if already present
        # DEV: is there ever a case where we'd NOT want it to be present?
        self.topology.add_node(
            subprim_handle
        )  # TODO: move to spearate method and add debug LOGGER output

        # register connections - NOTE: order matters here!
        # need to insert all connections, then pair up the internal ones
        ...
        for subprim_conn_handle, (
            nb_handle,
            nb_conn_handle,
        ) in neighbor_connections.items():
            self.connect_children(
                subprim_handle, subprim_conn_handle, nb_handle, nb_conn_handle
            )
        LOGGER.info(
            f"Attached child Primitive '{subprim_handle}' "
            f"to parent Primitive '{self._repr_brief()}'"
        )
    ...
    def _post_attach(self, parent: "Primitive") -> None:
        ...
        LOGGER.debug(
            f"Primitive '{parent._repr_brief()}' assigned as "
            f"parent of Primitive '{self._repr_brief()}'"
        )
    ...
    def _pre_detach(self, parent: "Primitive") -> None:
        """Preconditions prior to detachment of this Primitive from a parent"""
    ...
    def detach_child(
        ...
        assert self.num_internal_connections_on_child(target_handle) == 0, (
            f"Failed to disconnect all internal connections "
            f"on child Primitive '{target_handle}'"
        )

        # remove external connections on self (corresponding 1:1
        # with those on target after internal disconnection)
        assert (
            self.num_external_connectors_on_child(target_handle)
            == target_child.functionality
        ), (
            f"Failed to track all external connections "
            f"to child Primitive '{target_handle}'"
        )
        for target_conn_handle, own_conn_handle in self.external_connectors_on_child(
            target_handle
        ).items():
            self.unbind_external_connector(own_conn_handle)

        # discard from topology (raises Exception if not present in topology)
        self.topology.remove_node(
            target_handle
        )  # TODO: move to spearate method and add debug LOGGER output
        ...
        LOGGER.info(
            f"Detached child Primitive '{target_handle}' "
            f"from parent Primitive '{self._repr_brief()}'"
        )
    ...
    def _post_detach(self, parent: "Primitive") -> None:
        """Post-actions to take once attachment is verified and parent is bound"""
        LOGGER.debug(
            f"Primitive '{parent._repr_brief()}' disowned "
            "former child Primitive {self._repr_brief()}"
        )
    ...
    def disconnect_children(
        ...
        """
        Disconnect all internal connections between a pair
        of children, making the associated Connectors external
        """
        assert child_1_handle != child_2_handle, (
            "Cannot disconnect a Primitive from itself"
        )
        internal_conn_ref = self.internal_connections_by_pairs.get(
            frozenset((child_1_handle, child_2_handle)), None
        )
        if internal_conn_ref is None:
            LOGGER.warning(
                "No internal connections exist between "
                "the given pair of children; nothing to disconnect"
            )
            return
    ...
    def check_children_bijective_to_topology_nodes(
        ...
        if topology.number_of_nodes() != self.num_children:
            raise BijectionError(
                f"Cannot bijectively map {self.num_children} child Primitives "
        ...
        if node_labels != child_handles:
            raise BijectionError(
                f"Set underlying topology does not correspond to handles on "
                f"child Primitives; {len(node_labels - child_handles)} element(s) "
                f" present w/o associated children, and "
                f"{len(child_handles - node_labels)} child Primitive(s) "
                "are unrepresented in the topology"
            )

    def check_internal_connections_bijective_to_topology_edges(
        self, topology: TopologicalStructure
    ) -> None:
        """
        Verify that a 1:1 correspondence exists between the internal connections
        I.e. Connectors paired between sibling child Primitives
        and the edges present in the incidence topology
        """
        if topology.number_of_edges() != self.num_internal_connections:
            raise BijectionError(
                f"Cannot bijectively map {self.num_internal_connections} internal "
                f"connections onto {topology.number_of_edges()}-edge topology"
            )

        edge_labels = set(
            frozenset(edge) for edge in topology.edges
        ...
        if edge_labels != internal_conn_pairs:
            raise BijectionError(
                f"Incident pairs in associated topology do not correspond "
                "to internally-connected pairs of child Primitives;"
                f"{len(edge_labels - internal_conn_pairs)} edge(s) have no "
                "corresponding connection and {len(internal_conn_pairs - edge_labels)} "
                "internal connection(s) are unrepresented in the topology"
            )
    ...
    def set_topology(
        self, new_topology: TopologicalStructure, max_registration_iter: int = 100
    ) -> None:
        """
    ...
    def adjoin_child_nodes(
        ...
        **edge_attrs,
        ...
        LOGGER.debug(
            f"Added edge between child Primitives '{child_1_handle}' and "
            f"'{child_2_handle}' in topology of {self._repr_brief()}"
        )
    ...
    def set_connectivity_from_topology(
        ...
        LOGGER.warning(
            ...
            " User should verify the connections assigned make sense!"
        )

        internal_connections_inferred: dict[
            frozenset[PrimitiveHandle], frozenset[ConnectorReference]
        ] = infer_connections_from_topology(
            topology=topology,
            mapped_connectors={
        ...
        self.check_internal_connections_bijective_to_topology_edges(
            topology
        )  # verify that all edges have been accounted for

    # Resolution shift methods
    # TODO: make out-of-place versions of these methods (e.g. via optional_in_place)?
    ...
    def contract(
        ...
        #    raise ValueError(
        #        f"Cannot contract child Primitives into new intermediate with label "
        #        f"'{master_label}' already present amongst children"
        #    )

        # if not target_labels.issubset(self.unique_child_labels):
        #     raise ValueError(
        #         "Child Primitives labels chosen for contraction are "
        #         "not a proper subset of the children actually present"
        #     )
    ...
    def expand(
        ...
        # 1) determine which connections (with prior handles) to
        # re-establish once target is replaced with its children.
        # MUST be done first, as re-attaching and remapping
        # grandchildren will destroy these handles on the target
        LOGGER.info(
            f"Expanding child Primitive '{target_handle}' of {self._repr_brief()}, "
            f"replacing it with its {child_primitive.num_children} children"
        )
        # necessary to be satisfied for reconnection operations to be well-defined
        self.check_self_consistent()

        prior_internal_connections: set[frozenset[ConnectorReference]] = set(
            child_primitive.internal_connections
        )
        # NOTE: done as dict to delineate which handles DO need to be remapped
        # n(namely keys) vs those which don't (namely those of unchanged neighbors)
        prior_neighbor_connections_map: dict[ConnectorReference, ConnectorReference] = {
            ...
            for child_conn_handle, nb_conn_ref in self.internal_connections_on_child(
                target_handle
            ).items()
        }

        # 2) detach target from self, attaching its children
        # ("grandchildren" of self) in its place
        handle_remap: dict[PrimitiveHandle, PrimitiveHandle] = {}
        child_primitive_detached = self.detach_child(
            target_handle
        )  # detach target from self

        # 2a) compatibilize Connectors by collapsing correspondent pairs from external
        # Connectors into single, representative Connector on each grandchild
        for (
            child_conn_handle,
            grandchild_conn_ref,
        ) in child_primitive_detached.external_connectors.items():
            child_conn = child_primitive_detached.fetch_connector(child_conn_handle)
            grandchild = child_primitive_detached.fetch_child(
                grandchild_conn_ref.primitive_handle
            )
            grandchild_conn_ref_label, grandchild_conn_ref_index = (
                grandchild_conn_ref.connector_handle
            )
            grandchild_conn = grandchild.connectors.deregister(
                grandchild_conn_ref.connector_handle
            )
            ...
            new_grandchild_conn_handle = grandchild.register_connector(
                ...
                # use identical label to immediately recover
                # handle from "freed" buffer in internal registry
                label=grandchild_conn_ref_label,
            )
            assert new_grandchild_conn_handle == grandchild_conn_ref.connector_handle, (
                "Connector handle changed unexpectedly during merging"
            )

        # 2b) reassign grandchildren as direct children of self and discard
        # twitching corpse of target child (we have no further use for it from here on)
        for (
            old_handle,
            grandchild,
        ) in child_primitive_detached.children_by_handle.items():
            # attach and map handle
            # DEV: worth explicitly detaching grandchildren from target?
            old_label, old_idx = old_handle
            handle_remap[old_handle] = self.attach_child(grandchild, label=old_label)

        # 3) re-map previously-established connections to updated handles
        promised_connections: set[frozenset[ConnectorReference]] = set()
        for (
            prior_internal_conn_ref1,
            prior_internal_conn_ref2,
        ) in prior_internal_connections:
            promised_connections.add(
                frozenset((
                    prior_internal_conn_ref1.with_reassigned_primitive(
                        handle_remap[prior_internal_conn_ref1.primitive_handle]
                    ),
                    prior_internal_conn_ref2.with_reassigned_primitive(
                        handle_remap[prior_internal_conn_ref2.primitive_handle]
                    ),
                ))
            )
        for (
            prior_external_conn_ref,
            unaltered_nb_conn_ref,
        ) in prior_neighbor_connections_map.items():
            promised_connections.add(
                frozenset((
                    prior_external_conn_ref.with_reassigned_primitive(
                        handle_remap[prior_external_conn_ref.primitive_handle]
                    ),
                    unaltered_nb_conn_ref,
                ))
            )
        ...
        # verify that all parts are consistent once the dust settles
        self.check_self_consistent()

    # DEV: eventually wrap with optional_in_place, once I've
    # sorted how to provide custom copy method in general?
    def expanded(
        self,
        target_handle: PrimitiveHandle,
        connector_selector: ConnectorSelector=make_second_resemble_first,
    ...
    def flatten(self) -> None:
        """Flatten hierarchy under this Primitive, so the entire tree has height 1"""
        while (
            target_handles := self.expandable_children
        ):  # DEV: also works for simple Primitives (no casework needed!)
    ...
    def shape(self, new_shape: BoundedTransformableShape) -> None:
        ...
        if not isinstance(new_shape, BoundedTransformableShape):
            raise TypeError(
                "Primitive shape must be BoundedTransformableShape instance, "
                f"not object of type {type(new_shape).__name__}"
            )

        # NOTE: make copy to avoid mutating original (Principle of Least Astonishment)
        new_shape_clone = new_shape.copy()
        if self._shape is not None:
            new_shape_clone.cumulative_transformation = (
                self._shape.cumulative_transformation
            )  # transfer translation history BEFORE overwriting
    ...
    def _copy_untransformed(self) -> "Primitive":
        ...
        clone_primitive = self.__class__(
            ...
            # NOTE: connectors and children transferred verbatim below
            # no need to set in init here
            connectors=None,
            children=None,
            label=self.label,
        ...
        clone_primitive._internal_connections = set(
            self._internal_connections
        )  # frozensets are immutable, so no need to copy deeper
        ...
        for subprimitive in clone_primitive.children_by_handle.values():
            # needs to be rebound, since bypassing attach_child() to preserve handles
            subprimitive.parent = clone_primitive
    ...
    # Comparison methods
    def __hash__(self):
        # Hashing needs to be stricter than equality, i.e. two Primitives
        # may be distinguishable by hash, but nevertheless equivalent
        """Hash used to compare Primitives for identity (NOT equivalence)"""
        # return hash(self.canonical_form())
        return hash(self.canonical_form_peppered())

    def __eq__(self, other: object) -> bool:
        # DEVNOTE: in order to use equivalent-but-not-identical Primitives
        # as nodes in nx.Graph, __eq__ CANNOT evaluate similarity by hashes
    ...
    def coincident_with(self, other: "Primitive") -> bool:
        """
        Check whether two Primitives are coincident (i.e. all spatial parts
        are either equally unassigned or occupy the same space)
        """
        raise NotImplementedError

    def equivalent_to(self, other: "Primitive") -> bool:
        """
        Check whether two Primitives are equivalent (i.e. have interchangeable
        parts which are not necessarily in the same place in space)
        """
        raise NotImplementedError
    ...
    def __repr__(self) -> str:
        ...
        attr_str = ", ".join(
            f"{attr}={value_str}" for (attr, value_str) in repr_attr_strs.items()
        )
    ...
    # TODO: move this to external util
    def hierarchy_summary(
        ...
        """
        style_aliases: dict[str, AbstractStyle] = {
            "cont": ContStyle(),
            "Cont": ContStyle(),
            "continued": ContStyle(),
            "ContStyle": ContStyle(),
    ...
    def _hierarchy_tree(
        ...
        """
        Generate directed graph representing the connectivity
        of this Primitive and all its descendants
        """
        ...
        for child_handle, child in self.children_by_handle.items():
            hier_tree = nx.compose(
                ...
                ## TODO: add mechanism for de-duplifying handles by depth
                ## (related to planned expansion acceleration)
                # rename=(),
            )
