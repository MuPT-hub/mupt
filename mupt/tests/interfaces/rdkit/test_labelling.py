...
from mupt.interfaces.rdkit.labelling import RDMOL_NAME_PROP_PRECEDENCE, name_for_rdkit_mol
...
@pytest.mark.parametrize('prop', RDMOL_NAME_PROP_PRECEDENCE)
...
@pytest.mark.skipif(len(RDMOL_NAME_PROP_PRECEDENCE) < 2, reason="Not enough precedence levels to test")
def test_higher_precedence_overrides_lower(mol : Chem.Mol) -> None:
    ...
    primary_prop_attr = RDMOL_NAME_PROP_PRECEDENCE[0]
    secondary_prop_attr = RDMOL_NAME_PROP_PRECEDENCE[-1]
