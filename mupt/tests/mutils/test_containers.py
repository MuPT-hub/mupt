...
) -> list[tuple[UniqueRegistry, Hashable]]:
    """Convenience method for unpacking ticker keys while avoiding sharing state"""
    reg = reg_factory()
    return [(reg_factory(), key) for key in reg._ticker]
...
def test_reset_ticker_indiv(reg: UniqueRegistry, key: LabelT) -> None:
    ...
    orig_ticker: dict[LabelT, int] = {  # making deep copy to avoid value contamination
        key: count for key, count in reg._ticker.items()
    }
    other_keys = set(reg._ticker.keys()) - {key}

    reg.reset_ticker_count_for(key)
    assert reg._ticker[key] == 0 and all(
        reg._ticker[other_key] == orig_ticker[other_key] for other_key in other_keys
    )
...
def test_split() -> None:
    ...
    subregs = {
        category: dict(subreg) for category, subreg in subregs.items()
    }  # conversion done purely for easy comparison
