...
def compile_element_to_atom_params() -> dict[ElementLike, Atom]:
    ...
    labelled_linker.SetIsotope(
        34
    )  # test that arbitrary isotope can be applied to linker atoms
    ...
    linker_anion.SetFormalCharge(
        -1
    )  # DEV: not sure why you'd ever want this, but it IS technically supported
...
def compile_atom_to_element_params() -> dict[Atom, ElementLike]:
    ...
    return {atom: elem for elem, atom in compile_element_to_atom_params().items()}


@pytest.mark.parametrize(
    "element, atom_expected", compile_element_to_atom_params().items()
)
...
@pytest.mark.parametrize(
    "atom, element_expected", compile_atom_to_element_params().items()
)
