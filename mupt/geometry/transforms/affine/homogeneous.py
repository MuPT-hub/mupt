...
def to_homogeneous_coords(
    ...
    pad_widths[-1, -1] = (
        1  # ...EXCEPT exactly one layer AFTER the final dimension (and 0 before)
    )

    return np.pad(
        positions, pad_width=pad_widths, mode="constant", constant_values=projection
    )
...
def from_homogeneous_coords(
    ...
    """
    return (
        positions[..., :-1] / positions[..., -1, None]
    )  # strip off and normalize by the projective part (via broadcast)
