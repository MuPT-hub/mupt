...
def alignment_rotation(
    ...
    rotation_matrix = reflector(onto_vector) @ reflector(
        bisector
    )  # onto_vector doesn't need to be normalized
    assert is_orthogonal(rotation_matrix), (
        "Calculated alignment is not a proper rotation"
    )
