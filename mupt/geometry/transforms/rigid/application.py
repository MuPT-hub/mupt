...
class RigidlyTransformable(Copyable, Protocol):
    ...
    # AND made similar considerations for proposed changes
    ...
    def cumulative_transformation(self) -> RigidTransform:
        ...
        Heuristically, describes "how much has this object moved from where it started"

        Used for tracking provenance of rigid transformations
    ...
    def _copy_untransformed(self) -> Self:
        """
        Defines how to make a copy of an object with the same internal
        parts, but  without preserving it's cumulative transformation
        """
        raise NotCopyableError(
            f"Class {self.__class__.__name__} does not define "
            "how instances should copy their parts"
        )

    def copy(self) -> Self:
        """
        Make a copy of this RigidlyTransformable object,
        with transformation history preserved
        """
        new_obj = self._copy_untransformed()
        new_obj.cumulative_transformation = (
            self.cumulative_transformation
        )  # transfer net displacement WITHOUT directly transforming copied parts
    ...
    def rigidly_transformed(self, transformation: RigidTransform) -> Self:
        """
        Return a copy of this object which has been transformed
...
def apply_rigid_transformation_recursive(
    ...
    -------
    Any
        The transformed object, which (depending on the transformability
        and return types of the input and its members) may or many not be
        of the same type as the initial object
    """
