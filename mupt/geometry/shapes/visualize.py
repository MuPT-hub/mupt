...
def visualize_shape(
    ...
    **kwargs
    ...
    if ax is None:
        ...
        raise TypeError(
            f"Require matplotlib Axes-like for shape mesh drawing, not {type(ax).__name__}"
        )
    elif not isinstance(ax, Axes3D):
        raise ValueError(
            "Provided Axes are not 3D, and cannot support shape mesh drawing"
        )
    ...
    valid_mesh_kws: set[str] = signature(shape.surface_mesh).parameters.keys() - {
        "self"
    }
    mesh_kwargs: dict[str, Any] = {
        kw: kwargs.pop(kw) for kw in valid_mesh_kws if kw in kwargs
    }
    ...
    ax.set_aspect(
        "equal"
    )  # avoids scaling distortion along axes (otherwise, Ellipsoids would plot like Spheres)
