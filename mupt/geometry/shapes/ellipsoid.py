...
class Ellipsoid(BoundedTransformableShape):
    ...
    def is_valid_ellipsoid_matrix(basis: Array4x4) -> bool:
        ...
        return bool(
            is_columnspace_mutually_orthogonal(
                axes
            )  # ensure principal axes are mutually orthogonal
            and np.allclose(
                projective_part, 0.0
            )  # ensure axes have apply no projective transformation
            and np.isclose(
                w, 1.0
    ...
    def scaling_matrix(self, as_affine: bool = True) -> Array3x3 | Array4x4:
        ...
        if as_affine:
            return np.diag([
                *self.radii,
                1.0,
            ])  # add a 1.0 for the homogeneous coordinate
    ...
    def as_affine_matrix(self) -> Array4x4:
        ...
        """
        return self.cumulative_transformation.as_matrix() @ self.scaling_matrix(
            as_affine=True
        )
    ...
    def affine_inverse(self) -> Array4x4:
        ...
        return np.linalg.inv(
            self.as_affine_matrix()
        )  # precompute inverse for later use
