...
def random_orthogonal_vector(
    ...
    while not are_linearly_independent(
        random_direction, vector
    ):  # rejection sampling avoids colinear vector edge case
        random_direction = random_vector(
            ndim, low=-1.0, high=1.0, normalized=True, rng=rng
        )

    tangent = rejector(vector) @ random_direction
    if normalized:
        normalize(tangent)
    assert np.isclose(np.dot(tangent, vector), 0.0), (
        "Calculation error; tangent vector not orthogonal to input vector"
    )
