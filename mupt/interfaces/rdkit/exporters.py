...
def primitive_to_rdkit(
    ...
    mol = RWMol()
    conf = Conformer(
        primitive.num_children + primitive.functionality
    ...
    for (
        ...
        primitive.children_by_handle.items()
    ):  # at this point after flattening, all children should be atomic Primitives
        ...
        conf.SetAtomPosition(
            idx,
            child_prim.shape.centroid
            if (child_prim.shape is not None)
            else default_atom_position[:],
    ...
    for conn_ref in primitive.external_connectors.values():  # TODO: generalize to work for atomic (i.e. non-hierarchical) Primitives w/o external_connectors
        ...
        mol.AddBond(
            atom_idx_map[conn_ref.primitive_handle], linker_idx, order=conn.bondtype
        )
        conf.SetAtomPosition(
...
def primitive_to_rdkit_mols(
    ...
    first_residue_idx = 0
    for segment_idx, data in enumerate(
        strategy.iter_mol_data(primitive, resname_map=resname_map)
    ):
