...
class DPDRandomWalk(PlacementGenerator):
    ...
    def _generate_placements(
        ...
        hoomd_chains: dict[int, tuple[int]] = (
            dict()
        ...
        for chain_idx, chain in enumerate(primitive.topology.chains):
            ...
            for bead_handle in path:
                is_terminal: bool = (bead_handle == head_handle) or (
                    bead_handle == tail_handle
                )
                ...
                for conn_handle, conn in bead_prim.connectors.items():
                    traver_dir: TraversalDirection = next(
                        att
                        for att in conn.anchor.attachables
                        if isinstance(att, TraversalDirection)
                    )
                    traver_dir_idx: dict[TraversalDirection, int] = {
                        TraversalDirection.ANTERO: 0,
                        TraversalDirection.RETRO: 1,
                    }
                    anchor_positions[traver_dir_idx[traver_dir], :] = (
                        conn.anchor.position
                    )
                    if is_terminal:
                        radial_vector = conn.anchor.position - bead_prim.shape.centroid
                        diametric_anchor_pos = bead_prim.shape.centroid - radial_vector
                        anchor_positions[
                            traver_dir_idx[TraversalDirection.complement(traver_dir)], :
                        ] = diametric_anchor_pos
            ...
            frame.particles.position[handle_to_particle_idx[head_handle]] = (
                np.random.uniform(  # place head randomly within box bounds
                    low=(-L / 2),
                    high=(L / 2),
                    size=3,
                )
            ...
            for prim_handle_outgoing, prim_handle_incoming in sliding_window(path, 2):
                idx_outgoing, idx_incoming = idx_pair = (
                ...
                frame.particles.position[idx_incoming] = (
                    frame.particles.position[idx_outgoing] + delta
                )
        ...
        while not check_inter_particle_distance(
            ...
            if (total_steps_run % self.report_interval) == 0:
                LOGGER.debug(
                    f"Integrated {total_steps_run} steps; continuing simulation"
                )
        ...
        orient_marker_points = np.zeros(
            (frame.particles.N, 3, 3), dtype=float
        )  # each 3x3 slice store incoming-center-outgoing pos for particles
        for chain_idx, particle_indices in hoomd_chains.items():
            chain_particle_centers = positions_scaled[particle_indices, :]
            chain_radii = np.array([
        ...
        for handle, particle_idx in handle_to_particle_idx.items():
            point_incoming, point_center, point_outgoing = orient_marker_points[
                particle_idx, :, :
            ]
            reference_incoming, reference_outgoing = reference_anchor_positions[
                particle_idx
            ]

            secant_vector = (
                point_outgoing - point_incoming
            )  # span chord within spehrical bead between inter-bead anchor points
