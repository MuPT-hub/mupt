...
def random_walk_jointed_chain(
    ...
    if (
        not -np.pi <= clip_angle <= np.pi
    ):  # negative angles are allowed since cosine is even
    ...
    if (
        initial_point.shape != (dimension,)
    ):  # NOTE: check user-provided start shape or (redundantly-but-safely) the shape of the auto-assigned start)
        raise ValueError(
    ...
    assert initial_direction.shape == (
        dimension,
    )  # NOTE: check user-provided start direction shape

    if n_steps_max is None:
        if not isinstance(step_size, Iterable):
            LOGGER.warning(
                "No upper bound supplied for number of random walk steps; singly-valued step size will produce steps indefinitely!"
            )
        elif not isinstance(step_size, Sized):
            LOGGER.warning(
                "No upper bound supplied for number of random walk steps; unbounded step size iterator *MAY* produce steps indefinitely!"
            )

    # generate walk points
    n_steps_taken: int = 0
    net_position: np.ndarray = np.array(
        initial_point
    )  # make mutable copy of (possibly-immutable) initial point (will be incremented as net distance is tracked)
    ...
    for step_size in flexible_iterator(step_size, allowed_types=(Number,)):
        ...
        while (
            np.dot(step_direction, prev_direction) < cos_max
        ):  # NOTE: over |x| in [0, pi], cos(x) is monotonically decreasing, so overly-large steps will have cosine BELOW the cutoff
...
class AngleConstrainedRandomWalk(PlacementGenerator):
    ...
    def get_termini_handles(
        self, chain: TopologicalStructure
        ...
        if len(termini) == 2:
            ...
            raise ValueError(
                "Unbranched topology must have either 1 or 2 terminal nodes"
            )
    ...
    def check_preconditions(self, primitive: Primitive) -> None:
        ...
        if any((subprim.shape is None) for subprim in primitive.children):
            raise TypeError(
                "Random walk chain builder requires ellipsoidal of spherical beads to determine step sizes"
            )
    ...
    def _generate_placements(
        ...
        for chain in primitive.topology.chains:
            ...
            for prim_handle_outgoing, prim_handle_incoming in sliding_window(path, 2):
                conn_handle_outgoing, conn_handle_incoming = (
                    primitive.internal_connection_between(
                ...
                conn_outgoing = primitive.fetch_connector_on_child(
                    prim_handle_outgoing, conn_handle_outgoing
                )
                ...
                conn_incoming = primitive.fetch_connector_on_child(
                    prim_handle_incoming, conn_handle_incoming
                )
                ...
                Connector.mutually_antialign_ballistically(
                    conn_outgoing, conn_incoming
                )  # align linkers w/ other's anchor while leaving anchors themselves undisturbed
            ...
            rw_steps: Generator[np.ndarray, None, None] = random_walk_jointed_chain(
                ...
                n_steps_max=len(
                    path
                ),  # not strictly necessary, but suppresses "indeterminate num steps" warnings
            ...
            for handle, (step_start, step_end) in zip(
                path, sliding_window(rw_steps, 2)
            ):
                LOGGER.debug(
                    f"Random walk placing body {handle} along vector from {step_start} to {step_end}"
                )
                conn_start, conn_end = connection_points[handle]
                ...
                step_correction = 1 / (
                    1 + (self.bond_length / full_step_len)
                )  # scale back to account for bond length being included in step size
                t_step = (
                    0.5 * step_correction
                )  # adjust step fraction to account for bond length (will always be strictly smaller than 0.5, since ratio of lengths is positive)
