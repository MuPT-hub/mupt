...
class PlacementGenerator(ABC):
    ...
    Defines interface for generating centers & orientations
    (referred to as "placements" here), represented by RigidTransform
    instances, for all children of a given Primitive
    """
    ...
    # DEV: deliberately NOT abstract, as some builders may opt to
    # simply not check anything (establish this as the default behavior)
    ...
    def generate_placements(
        ...
        """
        Accepts a Primitive (with associated topology, connections, and
        hierarchy of children below it) and should generate placements
        (as RigidTransform instances) for all children below the provided
        Primitive, identified by the handles of each child Primitive
        """
